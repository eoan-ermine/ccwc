...
constexpr auto bytesCount(AccumulatorType &accumulator, BufferType &buffer, size_t nBytes) {
    accumulator += nBytes;
}

constexpr auto linesCount(AccumulatorType &accumulator, BufferType &buffer, size_t nBytes) {
    for (char *p = buffer.data(); (p = (char *) memchr(p, '\n', (buffer.data() + nBytes) - p)); ++p)
        ++accumulator;
};

constexpr auto wordsCount(AccumulatorType &accumulator, BufferType &buffer, size_t nBytes) {
    bool isSpace = false;
...
constexpr auto multibyteCount(AccumulatorType &accumulator, BufferType &buffer, size_t nBytes) {
